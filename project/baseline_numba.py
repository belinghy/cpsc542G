...
def _advect(w, wbuf,
    ...
    for iz in range(wh):
        for ix in range(ww):
            ...
            z2 = z - 0.5 * timestep * K1v

            K2u = _mac_interpolate(x2, z2, u, uw, uh, uox, uoz) / wdx
            K2v = _mac_interpolate(x2, z2, v, vw, vh, vox, voz) / wdx
            x3 = x2 - 0.5 * timestep * K2u
            z3 = z2 - 0.5 * timestep * K2v

            K3u = _mac_interpolate(x3, z3, u, uw, uh, uox, uoz) / wdx
            K3v = _mac_interpolate(x3, z3, v, vw, vh, vox, voz) / wdx
            x4 = x3 - timestep * K3u
            z4 = z3 - timestep * K3v

            K4u = _mac_interpolate(x4, z4, u, uw, uh, uox, uoz) / wdx
            K4v = _mac_interpolate(x4, z4, v, vw, vh, vox, voz) / wdx
            x -= timestep * ((1/6)*K1u + (2/6)*K2u + (2/6)*K3u + (1/6)*K4u)
            z -= timestep * ((1/6)*K1v + (2/6)*K2v + (2/6)*K3v + (1/6)*K4v)
